...
const AddDietDay = () => {
  ...
  useEffect(() => {
    axios.get("http://localhost:8080/api/general/get_all_meals", {
      headers: { Authorization: `Bearer ${token}` },
    }).then(res => setMeals(res.data.meals))
      .catch(err => console.error("Ошибка загрузки блюд", err));
    
    axios.get("http://localhost:8080/api/general/get_diet_types", {
      headers: { Authorization: `Bearer ${token}` },
  ...
  const handleSubmit = () => {
    ...
    axios.post("http://localhost:8080/api/admin/add_diet_day", requestData, {
      headers: { Authorization: `Bearer ${token}` },
    })
  ...
  return (
    <div className="p-4 max-w-lg mx-auto">
      ...
      <TextField fullWidth label="Название" value={name} onChange={(e) => setName(e.target.value)} margin="normal" />
      ...
      {Object.keys(selectedMeals).map(position => (
        <div key={position} className="mt-4">
          ...
          <FormControl fullWidth>
            ...
            <Select onChange={(e) => handleAddMeal(position, e.target.value, 100)}>
              <MenuItem value="">Выберите блюдо</MenuItem>
              {meals.map((meal) => (
                <MenuItem key={meal.id} value={meal.id}>{meal.name} ({meal.calories} ккал)</MenuItem>
              ))}
            </Select>
          </FormControl>
        </div>
      ...
      <Button variant="contained" color="primary" className="mt-4" onClick={handleSubmit}>Добавить</Button>
    </div>
  );
};
